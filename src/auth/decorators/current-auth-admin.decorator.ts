/**
 * CurrentAuthAdmin 参数装饰器
 *
 * 功能描述：
 * - 从当前请求上下文中提取认证信息（Auth），并校验是否为 Admin 身份。
 * - 若未认证或非 Admin，分别抛出 401 或 403 异常。
 *
 * 参数说明：
 * - data: any
 *   - 类型：any
 ...
 *
 * 返回值说明：
 * - 返回类型：Admin（GraphQL 生成类型）
 * - 含义：当前认证的管理员对象。
 *
 * 使用示例：
 * - GraphQL Resolver：
 *   @Query(() => Admin)
 *   profile(@CurrentAuthAdmin() admin: Admin) {
 *     return admin;
 *   }
 *
 * 注意事项：
 * - 当 request.authInfo 为空时抛出 UnauthorizedException（401）。
 * - 当 authInfo.admin 为空时抛出 ForbiddenException（403）。
...
export const CurrentAuthAdmin = createParamDecorator((data: any, ctx: ExecutionContext) => {
