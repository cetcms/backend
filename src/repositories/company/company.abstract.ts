...
export abstract class CompanyAbstract {
  ...
  private parseUpdateData(data: CompanyUpdateInput) {
    return CompanyUpdateInputObjectZodSchema.parse(data) as unknown as Prisma.CompanyUpdateInput;
  }
   ...
   * @private
   */
  private parseManyOrFirstArgs(args: FindManyCompanyArgs | FindFirstCompanyArgs) {
   ...
   * @returns 企业记录或null
   */
  findUnique(args: FindUniqueCompanyArgs): PrismaPromise<Company | null> {
    const include = this.getInclude();
    const where = this.parseUniqueWhere(args.where);
    return this.db.company.findUnique({ include, where });
  }
  ...
  findFirst(args: FindFirstCompanyArgs): PrismaPromise<Company | null> {
    ...
    return this.db.company.findFirst({ include, where, orderBy, skip, take, distinct, cursor });
  }
  ...
  findMany(args: FindManyCompanyArgs): PrismaPromise<Company[]> {
    const { include, where, orderBy, skip, take, distinct, cursor } = this.parseManyOrFirstArgs(args);
    return this.db.company.findMany({ include, where, orderBy, skip, take, distinct, cursor });
  }
   ...
   * @returns 记录数量
   */
  count(args: FindManyCompanyArgs): PrismaPromise<number> {
    const where = args.where ? this.parseWhere(args.where) : undefined;
    return this.db.company.count({ where });
  }
   ...
   * @returns 创建的企业记录
   */
  create(args: CreateOneCompanyArgs): PrismaPromise<Company> {
    const include = this.getInclude();
    const data = this.parseCreateData(args.data);
    return this.db.company.create({ include, data });
  }
   ...
   * @returns 更新后的企业记录
   */
  update(args: UpdateOneCompanyArgs): PrismaPromise<Company> {
    const include = this.getInclude();
    const where = this.parseUniqueWhere(args.where);
    const data = this.parseUpdateData(args.data);
    return this.db.company.update({ include, where, data });
  }
  ...
  upsert(args: UpsertOneCompanyArgs): PrismaPromise<Company> {
    ...
    return this.db.company.upsert({ include, where, create, update });
  }
   ...
   * @returns 删除的企业记录
   */
  delete(args: DeleteOneCompanyArgs): PrismaPromise<Company> {
    const include = this.getInclude();
    const where = this.parseUniqueWhere(args.where);
    return this.db.company.delete({ include, where });
  }
   ...
   * @returns 删除操作结果
   */
  deleteMany(args: DeleteManyCompanyArgs) {
    const where = args.where ? this.parseWhere(args.where) : undefined;
    const limit = args.limit ? args.limit : undefined;
    return this.db.company.deleteMany({ where, limit });
  }
}
