...
import {
  ...
  NotificationRecipientCreateInput,
  NotificationRecipientOrderByWithRelationInput,
  NotificationRecipientUpdateInput,
  NotificationRecipientWhereInput,
  NotificationRecipientWhereUniqueInput,
  CreateOneNotificationRecipientArgs,
  DeleteManyNotificationRecipientArgs,
  DeleteOneNotificationRecipientArgs,
  FindFirstNotificationRecipientArgs,
  FindManyNotificationRecipientArgs,
  FindUniqueNotificationRecipientArgs,
...
import {
  ...
  NotificationRecipientOrderByWithRelationInputObjectZodSchema,
  NotificationRecipientSelectObjectZodSchema,
  NotificationRecipientUpdateInputObjectZodSchema,
 ...
 * 消息通知接收数据访问抽象类
 *
 * 该抽象类提供了对消息通知接收数据的基本CRUD操作，包括查询、创建、更新、删除等功能。
 * 所有操作都通过Prisma ORM进行，并使用Zod进行数据验证。
 * 继承此类的具体实现需要提供handleParsedData方法的实现。
 */

@Injectable()
export abstract class NotificationRecipientAbstract {
  /**
   * 构造函数
   *
   ...
   * @returns 处理后的数据
   */
  protected abstract handleParsedData<
   ...
   * 用于指定查询时需要包含的关联数据
   */
  protected include: Prisma.NotificationRecipientInclude = {};
   ...
   * 用于指定查询时需要选择的字段
   */
  protected select: Prisma.NotificationRecipientSelect = {};
   ...
   * @returns 当前实例，支持链式调用
   */
  setInclude(include?: Prisma.NotificationRecipientInclude) {
    this.include = NotificationRecipientIncludeObjectZodSchema.parse(include) as Prisma.NotificationRecipientInclude;
    return this;
  }
   ...
   * @returns 当前实例，支持链式调用
   */
  setSelect(select?: Prisma.NotificationRecipientSelect) {
    this.select = NotificationRecipientSelectObjectZodSchema.parse(select) as Prisma.NotificationRecipientSelect;
    return this;
  }
   ...
   * @private
   */
  private parseWhere(where: NotificationRecipientWhereInput) {
    return NotificationRecipientWhereInputObjectZodSchema.parse(
      where
    ) as unknown as Prisma.NotificationRecipientWhereInput;
  }
   ...
   * @private
   */
  private parseUniqueWhere(where: NotificationRecipientWhereUniqueInput) {
    return NotificationRecipientWhereUniqueInputObjectZodSchema.parse(
      where
    ) as unknown as Prisma.NotificationRecipientWhereUniqueInput;
  }
   ...
   * @private
   */
  private parseCreateData(data: NotificationRecipientCreateInput) {
    return NotificationRecipientCreateInputObjectZodSchema.parse(
      data
    ) as unknown as Prisma.NotificationRecipientCreateInput;
  }
   ...
   * @private
   */
  private parseUpdateData(data: NotificationRecipientUpdateInput) {
    return NotificationRecipientUpdateInputObjectZodSchema.parse(
      data
    ) as unknown as Prisma.NotificationRecipientUpdateInput;
  }
  ...
  private parseOrderBy(
    ...
    if (Array.isArray(orderBy)) {
      return z
        .array(NotificationRecipientOrderByWithRelationInputObjectZodSchema)
        .parse(orderBy) as unknown as Prisma.NotificationRecipientOrderByWithRelationInput[];
    }
    return NotificationRecipientOrderByWithRelationInputObjectZodSchema.parse(
      orderBy
    ) as unknown as Prisma.NotificationRecipientOrderByWithRelationInput;
  }
   ...
   * @private
   */
  private parseManyOrFirstArgs<T extends FindManyNotificationRecipientArgs | FindFirstNotificationRecipientArgs>(
    args: T
  ) {
    return {
      ...args,
  ...
  findFirst(args: FindFirstNotificationRecipientArgs): PrismaPromise<NotificationRecipient | null> {
    ...
    const include = this.getInclude();
    const select = this.getSelect();
    return this.db.notificationRecipient.findFirst({
      ...(Object.keys(include).length > 0 && { include }),
      ...(Object.keys(select).length > 0 && { select }),
      ...args,
    });
  ...
  findMany(args: FindManyNotificationRecipientArgs): PrismaPromise<NotificationRecipient[]> {
    ...
    const include = this.getInclude();
    const select = this.getSelect();
    return this.db.notificationRecipient.findMany({
      ...(Object.keys(include).length > 0 && { include }),
      ...(Object.keys(select).length > 0 && { select }),
      ...args,
    });
   ...
   * @returns 删除操作结果
   */
  deleteMany(where?: DeleteManyNotificationRecipientArgs['where'], limit?: number) {
    return this.db.notificationRecipient.deleteMany({
      where: where ? this.parseWhere(where) : undefined,
      limit,
    });
  }
