 ...
 * 管理员数据访问仓库类
 *
 * 继承自AdminAbstract抽象类，实现了管理员数据的具体访问方法
 * 包括密码处理、按ID查询、按邮箱查询、密码验证等功能
 */

@Injectable()
export class AdminRepository extends AdminAbstract {
  /**
   * 构造函数
   *
   ...
   */
  save(where: UpsertOneAdminArgs['where'], data: UpsertOneAdminArgs['create']) {
    return this.upsert({
      where: where,
      update: data,
      create: data,
