...
export class WebsiteSeoService {
  ...
  async updatePages(auth: CurrentAuth, where: FindUniqueWebsiteArgs['where'], urls: string[]) {
    try {
      const { pages } = await this.siteInfo(auth, { where });
   ...
   * @private
   */
  private pushPagesToAnalyze(urls: string[]) {
    try {
  ...
  async sitePages(auth: CurrentAuth, args: FindUniqueWebsiteArgs, _withStatus = true): Promise<WebsiteSeoPage[]> {
