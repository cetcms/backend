...
export const main = async () => {
  ...
  for (const lang of languages) {
    ...
    for (const model of models) {
      ...
      const modelTitleKey = `title.${modelName}`;
      if (!get(translations[langFilePath], modelTitleKey)) {
        translations[langFilePath] = set(translations[langFilePath], modelTitleKey, modelName);
      }
